...
class Login:
    ...
    def __init__(self):
        ...
        self.canvas = Canvas(
            self.window, bd=0, highlightthickness=0, relief='ridge')
        self.canvas.pack(fill="both", expand="yes")

        self.canvas.create_image(0, 0, image=self.img, anchor="nw")

        self.ent1 = Entry(self.canvas, font=(
            'Helvetica', 24), width=12, bg="white")
        self.ent1.place(x=290, y=150)

        self.ent2 = Entry(self.canvas, font=('Arial', 24),
                          show="*", width=12, bg="white")
        self.ent2.place(x=290, y=218)

        self.btn = Button(self.canvas, image=self.login_btn_img,
                          bd=0, bg="#57E186", command=self.password_check)
...
class AdminPage:
    ...
    def __init__(self):
        ...
        result = self.cur.execute(
            f"SELECT * FROM login_log ORDER BY sr DESC LIMIT 1")
        for row in result:
        ...
        self.mark_att = LabelFrame(
            self.window, text="Mark Attendance", font='Arial 9 bold', bg="#dbffdb")
        self.mark_att.pack(fill='x', expand="yes", padx=10, pady=5)

        self.choose_sem_frame = LabelFrame(
            self.mark_att, text="Semester", font='Arial 9 bold', bg="#dbffdb")
        self.choose_sem_frame.pack(fill='x', expand="yes", padx=10, pady=5)

        self.sem1_btn = Button(
            self.choose_sem_frame, text=f"Semester {self.sem[0]}", command=self.sem1_btn_click)
        self.sem1_btn.grid(row=0, column=0, padx=10, pady=10)

        self.sem2_btn = Button(
            self.choose_sem_frame, text=f"Semester {self.sem[1]}", command=self.sem2_btn_click)
        self.sem2_btn.grid(row=0, column=1, padx=10, pady=10)

        self.choose_sub_frame = LabelFrame(
            self.mark_att, text="Subject", font='Arial 9 bold', bg="#dbffdb")
        self.choose_sub_frame.pack(fill='x', expand="yes", padx=10, pady=5)

        self.lbl4 = Label(self.choose_sub_frame,
                          text="Choose Subject", bg="#dbffdb")
        ...
        self.Mark_att_info = LabelFrame(
            self.mark_att, text="Mark Attendance info", font='Arial 9 bold', bg="#dbffdb")
        ...
        self.commands = LabelFrame(
            self.window, text="Commands", font='Arial 9 bold', bg="#dbffdb")
        ...
        self.logout_btn = Button(
            self.commands, text="Logout", bg="azure3", command=self.logout)
    ...
    def initialize_app(self):
        ...
        self.lbl5.config(
            text=f"Mark Attendance for {self.d2}, Semester {self.sem[0]}")
        self.sem_choice = self.sem[0]
        self.subjects = Subjects.subject_choose(
            self.sem[0], self.user_year, self.department)
        self.check_box()
    ...
    def sem2_btn_click(self):
        ...
        self.lbl5.config(
            text=f"Mark Attendance for {self.d2}, Semester {self.sem[1]}")
        self.subjects = Subjects.subject_choose(
            self.sem[1], self.user_year, self.department)
        ...
        try:
            for row in self.cur.execute(f"SELECT * FROM '{self.user_year}{self.department}attendance'"):
                if row[0] == self.d2 and row[2] == self.sub:
                    self.log_error = 1
                    messagebox.showerror(
                        "Error", f"Attendance is already marked\n{self.d2}, {self.sub}")
            if self.log_error == 0:
                self.window.destroy()
                MarkAttendance(self.user_year, self.department,
                               self.sem_choice, self.sub)
...
class MarkAttendance:
    ...
    def __init__(self, a, b, c, d):
        ...
        self.style.map("Treeview", background=[
                       ('selected', 'invalid', "#7fbacc")], foreground=[('selected', 'black')])

        self.att_info = LabelFrame(
            self.window, text="Attendance info", font='Arial 9 bold', bg="#dbffdb")
        self.att_info.pack(fill='x', expand="yes", pady=5, padx=10)

        self.lbl1 = Label(
            self.att_info, text=f"Department: {self.department}", bg="#dbffdb")
        ...
        self.lbl1 = Label(
            self.att_info, text=f"Subject: {self.sub}", bg="#dbffdb")
        self.lbl1.grid(row=1, column=1, padx=5, pady=5)

        self.lbl1 = Label(
            self.att_info, text=f"Semester: {self.sem}", bg="#dbffdb")
        ...
        self.my_tree.tag_configure(
            'checked', image=self.checked, background="#9dff9d")
        self.my_tree.tag_configure(
            'unchecked', image=self.unchecked, background="#ffa7a7")
    ...
    def abort(self):
        ...
        result = messagebox.askyesno(
            "Confirm exit", "Do you want to abort\nMarking attendance")
        if result:
            self.window.destroy()
            self.roll.clear()
            AdminPage()
    ...
    def save(self):
        ...
        result = messagebox.askyesno(
            "Confirm", "Do you want to mark attendance")
        if result:
    ...
    def check_if_empty(self):
        ...
        if not self.roll:
            result = messagebox.askyesno(
                "Caution", "No student marked present\nPress Yes to proceed")
            if result:
...
class AdminLog:
    ...
    def __init__(self, a, b, c):
        ...
        self.my_tree.heading("Present Roll no", anchor=W,
                             text="Present Roll no")

        self.button_frame = LabelFrame(
            self.window, text="Semester", font='Arial 9 bold')
        self.button_frame.config(bg="#dbffdb")
        self.button_frame.pack(fill="x", expand="yes", padx=10)

        self.sem1_btn = Button(
            self.button_frame, text=f"Semester {self.sem[0]}", command=self.sem1_btn_click)
        self.sem1_btn.grid(row=0, column=0, padx=10, pady=10)

        self.sem2_btn = Button(
            self.button_frame, text=f"Semester {self.sem[1]}", command=self.sem2_btn_click)
        self.sem2_btn.grid(row=0, column=1, padx=10, pady=10)

        self.back_btn = Button(
            self.button_frame, text="Back", bg="azure3", command=self.back_button)
        self.back_btn.grid(row=0, column=2, padx=10, pady=10)

        self.choose_sub = LabelFrame(
            self.window, text="Subject", font='Arial 9 bold')
        ...
        self.enter_btn = Button(
            self.choose_sub, text="Refresh Table", bg="azure3", command=self.input_check)
        ...
        self.user_commands = LabelFrame(
            self.window, text="Commands", font='Helvetica 9 bold', bg="#dbffdb")
    ...
    def input_check(self):
        if self.var.get() != '':
            ...
            messagebox.showerror(
                "Input error", "Subject input cannot be empty!\nChoose correct subject!")
